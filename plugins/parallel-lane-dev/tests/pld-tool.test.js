...
function writeFixture(root) {
  ...
  const executionDir = path.join(root, 'PLD', 'executions', 'demo-flow');
  const stateDir = path.join(root, 'PLD', 'state', 'demo-flow');
...
test('executor import-plans ingests legacy plans and cleans plan directory', () => {
  ...
  const dbPath = path.join(root, '.pld', 'executor.sqlite');
...
test('executor claim-assignment and report-result use the sqlite authority path', () => {
  ...
  assert.match(assignment.laneBranch, /^pld\/demo-flow\/lane-1$/);
...
test('legacy coordinator and dispatch helpers degrade to executor-backed summaries after migration', () => {
  ...
  const coordinator = JSON.parse(
    run(
      'node',
      [
        repoRoot('plugins', 'parallel-lane-dev', 'scripts', 'pld-run-coordinator-loop.cjs'),
        '--execution',
        'demo-flow',
        '--dry-run',
        '--json',
  ...
  const dispatchPlan = JSON.parse(
    run(
      'node',
      [
        repoRoot('plugins', 'parallel-lane-dev', 'scripts', 'pld-build-dispatch-plan.cjs'),
        '--execution',
        'demo-flow',
        '--dry-run',
        '--json',
...
test('legacy review and intake helpers read executor-backed lane outcomes after migration', () => {
  ...
  const review = JSON.parse(
    run(
      'node',
      [
        repoRoot('plugins', 'parallel-lane-dev', 'scripts', 'pld-drive-review-loop.cjs'),
        '--execution',
        'demo-flow',
        '--json',
      ],
  ...
  const intake = JSON.parse(
    run(
      'node',
      [
        repoRoot('plugins', 'parallel-lane-dev', 'scripts', 'pld-intake-ready-to-commit.cjs'),
        '--execution',
        'demo-flow',
        '--json',
      ],
...
test('legacy cycle and launch helpers emit executor-backed promotions after migration', () => {
  ...
  const cycle = JSON.parse(
    run(
      'node',
      [
        repoRoot('plugins', 'parallel-lane-dev', 'scripts', 'pld-run-cycle.cjs'),
        '--execution',
        'demo-flow',
        '--dry-run',
        '--json',
  ...
  const launch = JSON.parse(
    run(
      'node',
      [
        repoRoot('plugins', 'parallel-lane-dev', 'scripts', 'pld-launch-active-set.cjs'),
        '--execution',
        'demo-flow',
        '--dry-run',
        '--json',
  ...
  assert.match(launch.assignments[0].message, /Lane branch: pld\/demo-flow\/lane-1/);
});
...
test('legacy schedule and refill helpers read executor-backed state after migration', () => {
  ...
  const schedule = JSON.parse(
    run(
      'node',
      [
        repoRoot('plugins', 'parallel-lane-dev', 'scripts', 'pld-suggest-schedule.cjs'),
        '--execution',
        'demo-flow',
        '--json',
      ],
  ...
  const refill = JSON.parse(
    run(
      'node',
      [
        repoRoot('plugins', 'parallel-lane-dev', 'scripts', 'pld-suggest-refill.cjs'),
        '--execution',
        'demo-flow',
        '--lane',
        '1',
