...
function writeLaneState(root, execution, laneNumber, state) {
  const stateDir = path.join(root, 'PLD', 'state', execution);
...
function setupPldFixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-'));
  ...
  fs.mkdirSync(path.join(root, 'PLD', 'executions', 'plot-mode'), {recursive: true});
  fs.writeFileSync(
    path.join(root, 'PLD', 'executions', 'plot-mode', 'lane-1.md'),
    `# Lane 1 Plan - Node Contract and Handoff

> Ownership family:
> \`src/commands/root.ts\`
>
> PLD worktree: \`.worktrees/lane-1-node\`
>
...
`,
  ...
  fs.mkdirSync(path.join(root, 'PLD'), {recursive: true});
  fs.writeFileSync(
    path.join(root, 'PLD', 'scoreboard.md'),
    `# PLD Scoreboard
...
function setupSelfHostingScheduleFixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-schedule-'));
  const executionDir = path.join(root, 'PLD', 'executions', 'pld-self-hosting');
  fs.mkdirSync(executionDir, {recursive: true});

  const lanePlans = [
    {
      lane: 1,
      worktree: '.worktrees/pld-lane-1-scheduler',
      title: 'Scheduler Core',
      verification: 'node --test plugins/parallel-lane-dev/tests/pld-automation.test.js',
      item: 'Normalize scheduling phases for multi-lane dispatch',
    },
    {
      lane: 2,
      worktree: '.worktrees/pld-lane-2-scoreboard',
      title: 'Scoreboard Integration',
      verification: 'npm run pld:scoreboard:refresh',
      item: 'Keep scoreboard rows aligned with the active-cap model',
    },
    {
      lane: 3,
      worktree: '.worktrees/pld-lane-3-rules',
      title: 'Rules and Communication',
      verification: 'rg -n "active lane count" plugins/parallel-lane-dev/spec/PLD',
      item: 'Rewrite remaining fixed-lane wording',
    },
    {
      lane: 4,
      worktree: '.worktrees/pld-lane-4-tests',
    ...
    {
      lane: 5,
      worktree: '.worktrees/pld-lane-5-docs',
      title: 'Plot-Mode Migration',
      verification: 'rg -n "lane pool" PLD/executions/plot-mode',
      item: 'Adjust plot-mode docs to lane-pool language',
    },
    {
      lane: 6,
      worktree: '.worktrees/pld-lane-6-followup',
...
> Ownership family:
> \`plugins/parallel-lane-dev/tests/pld-automation.test.js\`
>
> PLD worktree: \`${lanePlan.worktree}\`
>
> Lane-local verification:
...
| Execution | Lane | Ownership | Current item | Phase | Item commit | Last verification | Blocked by | Next refill target | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| pld-self-hosting | Lane 1 | Scheduler core | Normalize scheduling phases for multi-lane dispatch | implementing | \`1a2b3c4\` | \`node --test plugins/parallel-lane-dev/tests/pld-automation.test.js\` | none | Scheduler edge cases | active lane |
| pld-self-hosting | Lane 2 | Scoreboard integration | Keep scoreboard rows aligned with the active-cap model | spec-review-pending | \`2a2b3c4\` | \`npm run pld:scoreboard:refresh\` | none | Scoreboard wording polish | active lane |
| pld-self-hosting | Lane 3 | Rules and communication | Rewrite remaining fixed-lane wording | refill-ready | \`3a2b3c4\` | \`rg -n "active lane count" plugins/parallel-lane-dev/spec/PLD; rg -n "4 active lanes" plugins/parallel-lane-dev/AGENTS.md\` | none | Execution wording cleanup | ready to refill |
| pld-self-hosting | Lane 4 | Regression and CLI surface | Add schedule regression coverage | refill-ready | \`4a2b3c4\` | \`node --test plugins/parallel-lane-dev/tests/pld-automation.test.js\` | none | Scoreboard/schedule cross-check coverage | ready to refill |
| pld-self-hosting | Lane 5 | Plot-mode migration | Adjust plot-mode docs to lane-pool language | queued | \`n/a\` | \`rg -n "lane pool" plugins/parallel-lane-dev/executions/plot-mode\` | wait-slot | Plot-mode overview wording | queued lane |
| pld-self-hosting | Lane 6 | Coordinator follow-up | Capture coordinator ergonomics follow-up | queued | \`n/a\` | \`sed -n '1,220p' tasks/todo.md\` | wait-slot | Coordinator follow-up | queued lane |
`,
    'utf8',
  );
...
test('scoreboard refresh v2 backfills effective phase and lane event metadata', () => {
  const fixture = setupPldFixture();
  process.env.PLD_PROJECT_ROOT = fixture.root;
  process.env.PLD_SCOREBOARD_PATH = path.join(fixture.root, 'PLD', 'scoreboard.md');
  process.env.PLD_RUNTIME_SCOREBOARD_PATH = path.join(
    fixture.root,
    'PLD',
    'state',
  ...
  const trackedBefore = fs.readFileSync(process.env.PLD_SCOREBOARD_PATH, 'utf8');
  const {updateScoreboard} = freshRequire('plugins/parallel-lane-dev/scripts/pld-refresh-scoreboard.cjs');
  updateScoreboard();

  const trackedAfter = fs.readFileSync(process.env.PLD_SCOREBOARD_PATH, 'utf8');
  const runtimeText = fs.readFileSync(process.env.PLD_RUNTIME_SCOREBOARD_PATH, 'utf8');
...
test('resolveProjectRoot prefers the current linked worktree when it has its own PLD surface', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-root-'));
  ...
  fs.mkdirSync(path.join(root, 'PLD'), {recursive: true});
  fs.writeFileSync(path.join(root, 'PLD', 'scoreboard.md'), '# root scoreboard\n', 'utf8');
  run('git', ['add', 'tracked.txt'], root);
  run('git', ['add', 'PLD/scoreboard.md'], root);
  run('git', ['commit', '-m', 'init'], root);
  run('git', ['worktree', 'add', path.join(root, '.worktrees', 'lane-1-node'), '-b', 'lane-1-node'], root);

  const originalCwd = process.cwd();
  const modulePath = path.join(originalCwd, 'plugins', 'parallel-lane-dev', 'scripts', 'pld-lib.cjs');
  delete process.env.PLD_PROJECT_ROOT;
  try {
    const worktreeRoot = path.join(root, '.worktrees', 'lane-1-node');
    fs.writeFileSync(path.join(worktreeRoot, 'PLD', 'scoreboard.md'), '# worktree scoreboard\n', 'utf8');
...
test('schedule helper prefers the current linked worktree PLD surface over the canonical repo root', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-worktree-'));
  fs.mkdirSync(path.join(root, '.worktrees'), {recursive: true});
  run('git', ['init', '-q'], root);
  run('git', ['config', 'user.name', 'Codex Test'], root);
  run('git', ['config', 'user.email', 'codex@example.com'], root);
...
`,
  ...
  const worktreeRoot = path.join(root, '.worktrees', 'lane-1-node');
  fs.writeFileSync(
    path.join(worktreeRoot, 'PLD', 'scoreboard.md'),
    `# PLD Scoreboard
...
`,
  ...
  fs.writeFileSync(
    path.join(worktreeRoot, 'PLD', 'executions', 'plot-mode', 'lane-1.md'),
    `# Lane 1
...
`,
  ...
  try {
    delete process.env.PLD_PROJECT_ROOT;
    delete process.env.PLD_SCOREBOARD_PATH;
    delete process.env.PLD_RUNTIME_SCOREBOARD_PATH;
    process.chdir(worktreeRoot);
    delete require.cache[require.resolve(modulePath)];
...
test('lane plans in a linked worktree still resolve shared .worktrees paths from the canonical worktree pool root', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-pool-root-'));
  fs.mkdirSync(path.join(root, '.worktrees'), {recursive: true});
  run('git', ['init', '-q'], root);
  run('git', ['config', 'user.name', 'Codex Test'], root);
  run('git', ['config', 'user.email', 'codex@example.com'], root);
...
`,
  ...
  fs.mkdirSync(path.join(recoveryRoot, 'PLD', 'executions', 'plot-mode'), {recursive: true});
  fs.writeFileSync(path.join(recoveryRoot, 'PLD', 'scoreboard.md'), '# recovery scoreboard\n', 'utf8');
  fs.writeFileSync(
    path.join(recoveryRoot, 'PLD', 'executions', 'plot-mode', 'lane-1.md'),
    `# Lane 1
...
`,
  ...
  try {
    delete process.env.PLD_PROJECT_ROOT;
    delete process.env.PLD_WORKTREE_POOL_ROOT;
...
test('probe helper reports source changes, artifact noise, and verification commands', () => {
  ...
  const {probeLane} = freshRequire('plugins/parallel-lane-dev/scripts/pld-probe-lane.cjs');
...
test('lane automation prefers execution-aware lane state journal over shared lane-number heuristics', () => {
  ...
  const {computeLaneAutomation} = freshRequire('plugins/parallel-lane-dev/scripts/pld-lib.cjs');
...
test('refill assistant suggests the next unchecked lane-local item for refill-ready lanes', () => {
  const fixture = setupPldFixture();
  const scoreboardPath = path.join(fixture.root, 'PLD', 'scoreboard.md');
  ...
  process.env.PLD_PROJECT_ROOT = fixture.root;
  process.env.PLD_SCOREBOARD_PATH = scoreboardPath;

  const {suggestRefill} = freshRequire('plugins/parallel-lane-dev/scripts/pld-suggest-refill.cjs');
...
test('self-hosting schedule keeps the active thread cap at four and dispatches refill-ready lanes first', () => {
  ...
  const {computeExecutionSchedule} = freshRequire('plugins/parallel-lane-dev/scripts/pld-lib.cjs');
  const {renderSchedule} = freshRequire('plugins/parallel-lane-dev/scripts/pld-suggest-schedule.cjs');

  const schedule = computeExecutionSchedule(fixture.root, 'pld-self-hosting', 4);
...
test('schedule suggestion prefers lane journal phase over stale scoreboard phase', () => {
  const fixture = setupSelfHostingScheduleFixture();
  writeLaneState(fixture.root, 'pld-self-hosting', 5, {
    execution: 'pld-self-hosting',
    lane: 'Lane 5',
    phase: 'refill-ready',
...
test('schedule and refill helpers prefer reduced envelope state over stale tracked rows', () => {
  ...
  const {recordEnvelope} = freshRequire('plugins/parallel-lane-dev/scripts/pld-envelope.cjs');
  const {suggestRefill} = freshRequire('plugins/parallel-lane-dev/scripts/pld-suggest-refill.cjs');
  const {computeExecutionSchedule} = freshRequire('plugins/parallel-lane-dev/scripts/pld-lib.cjs');
...
test('message helper renders correction-loop text without opening direct reviewer channels', () => {
  ...
  assert.match(
    message,
    /Return a new strict PLD lane handoff envelope JSON object, or READY_TO_COMMIT as eventType=ready-to-commit with proposed commit title\/body if coordinator commit is required/,
  );
...
test('message helper tells implementers to hand commit-ready mvc back to coordinator when commit may be gated', () => {
  ...
  assert.match(message, /return only one strict PLD lane handoff envelope JSON object/i);
...
test('lane state recorder writes execution-aware journal files for coordinator tooling', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-record-'));
  process.env.PLD_PROJECT_ROOT = root;

  const {recordLaneState} = freshRequire('plugins/parallel-lane-dev/scripts/pld-record-lane-state.cjs');
  const filePath = recordLaneState(root, {
    ...
    'commit-body': 'Add compare payload handoff',
    reviewer: 'PASS',
    'correction-count': 3,
  ...
  assert.equal(state.proposedCommitBody, 'Add compare payload handoff');
  ...
  const eventsPath = path.join(root, 'PLD', 'state', 'plot-mode', 'events.ndjson');
...
test('envelope recorder preserves command telemetry fields for command lifecycle events', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-command-envelope-'));
  process.env.PLD_PROJECT_ROOT = root;

  const {loadEnvelopeEvents, recordEnvelope} = freshRequire('plugins/parallel-lane-dev/scripts/pld-envelope.cjs');
  const cases = [
    {
  ...
  for (const entry of cases) {
    ...
    assert.equal(event.command, 'npm run pld:probe');
...
test('envelope recorder rejects invalid command block kinds', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-command-blockkind-'));
  process.env.PLD_PROJECT_ROOT = root;
...
test('command telemetry survives later ordinary lane updates without changing reviewer state', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-command-state-'));
  process.env.PLD_PROJECT_ROOT = root;
  ...
  const laneState = JSON.parse(
    fs.readFileSync(path.join(root, 'PLD', 'state', 'plot-mode', 'lane-1.json'), 'utf8'),
  );
  assert.equal(laneState.lastReviewerResult, 'pass');
  assert.equal(laneState.command, 'npm run pld:probe');
...
test('command telemetry helper records canonical envelopes and is wired to npm scripts', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-command-cli-'));
  const originalProjectRoot = process.env.PLD_PROJECT_ROOT;
  process.env.PLD_PROJECT_ROOT = root;

  try {
    const packageJson = JSON.parse(fs.readFileSync(repoRoot('package.json'), 'utf8'));
    assert.equal(
      packageJson.scripts['pld:command:record'],
      'node plugins/parallel-lane-dev/scripts/pld-record-command-event.cjs',
    );

    const executionDir = path.join(root, 'PLD', 'executions', 'plot-mode');
    fs.mkdirSync(executionDir, {recursive: true});
    fs.writeFileSync(
...
PLD worktree: \`.worktrees/lane-1-worker\`
...
| Execution | Lane | Ownership | Current item | Phase | Item commit | Last verification | Blocked by | Next refill target | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| plot-mode | Lane 1 | Worker telemetry | Record command lifecycle events | queued | \`n/a\` | \`node --test plugins/parallel-lane-dev/tests/pld-automation.test.js\` | none | n/a | telemetry lane |
`,
      'utf8',
    );

    const commandCli = repoRoot('plugins/parallel-lane-dev/scripts/pld-record-command-event.cjs');
    run(
      'node',
      [
        commandCli,
    ...
    assert.equal(recordedProbe.command, 'npm run pld:probe');
    ...
    if (originalProjectRoot === undefined) {
      ...
      process.env.PLD_PROJECT_ROOT = originalProjectRoot;
    }
  }
});

test('telemetry summarizer projects minute buckets and drop diagnostics from execution events', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-telemetry-'));
  ...
  const {telemetrySummaryPath, summarizeTelemetry} = freshRequire(
    'plugins/parallel-lane-dev/scripts/pld-summarize-telemetry.cjs',
  );
  ...
  assert.equal(summaryPath, path.join(root, 'PLD', 'state', execution, 'telemetry-summary.json'));
...
test('telemetry review renderer writes coordinator-readable markdown output', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-telemetry-review-'));
  const execution = 'plot-mode';
  process.env.PLD_PROJECT_ROOT = root;
  ...
  const {renderTelemetryReviewFile} = freshRequire(
    'plugins/parallel-lane-dev/scripts/pld-render-telemetry-review.cjs',
  );
...
test('telemetry summarizer keeps implementing lanes productive after command-finished and avoids generic blocked->dependency diagnosis', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-telemetry-edge-'));
  const execution = 'plot-mode';
  process.env.PLD_PROJECT_ROOT = root;
  ...
  const {summarizeTelemetry} = freshRequire('plugins/parallel-lane-dev/scripts/pld-summarize-telemetry.cjs');
  const summary = summarizeTelemetry(root, execution);
...
test('envelope reducer projects tracked scoreboard and lane status from a single handoff event', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-envelope-'));
  process.env.PLD_PROJECT_ROOT = root;

  fs.mkdirSync(path.join(root, 'PLD', 'executions', 'plot-mode'), {recursive: true});
  fs.writeFileSync(
    path.join(root, 'PLD', 'executions', 'plot-mode', 'lane-4.md'),
    `# Lane 4

> Ownership family:
> \`src/render/panels.rs\`
>
> PLD worktree: \`.worktrees/lane-4-panels\`
>
> Lane-local verification:
...
`,
  ...
  const {recordEnvelope, loadEnvelopeEvents} = freshRequire('plugins/parallel-lane-dev/scripts/pld-envelope.cjs');
  recordEnvelope(root, {
    execution: 'plot-mode',
    lane: 'Lane 4',
    role: 'implementer',
    ...
    'commit-title': 'feat(plot): clarify adopted routing target in compare panel',
    'commit-body': 'Make the compare panel clearly show the adopted routing target',
    insights: [
      {
        lane: 'Lane 4',
        source: 'subagent',
  ...
  const state = JSON.parse(
    fs.readFileSync(path.join(root, 'PLD', 'state', 'plot-mode', 'lane-4.json'), 'utf8'),
  );
  ...
  assert.equal(state.proposedCommitTitle, 'feat(plot): clarify adopted routing target in compare panel');

  const scoreboard = fs.readFileSync(path.join(root, 'PLD', 'scoreboard.md'), 'utf8');
  ...
  const lanePlan = fs.readFileSync(path.join(root, 'PLD', 'executions', 'plot-mode', 'lane-4.md'), 'utf8');
  ...
  const insightsLines = fs
    .readFileSync(path.join(root, 'PLD', 'state', 'plot-mode', 'execution-insights.ndjson'), 'utf8')
    .trim()
    .split('\n');
...
test('envelope replay uses the execution root when deriving fallback lane content', () => {
  const executionRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-envelope-root-'));
  const cwdRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-envelope-cwd-'));
  const originalCwd = process.cwd();
  const originalProjectRoot = process.env.PLD_PROJECT_ROOT;
  const modulePath = path.join(originalCwd, 'plugins', 'parallel-lane-dev', 'scripts', 'pld-envelope.cjs');

  try {
    fs.mkdirSync(path.join(executionRoot, 'PLD', 'executions', 'plot-mode'), {recursive: true});
    fs.writeFileSync(
      path.join(executionRoot, 'PLD', 'executions', 'plot-mode', 'lane-1.md'),
      `# Lane 1
...
`,
    ...
    fs.writeFileSync(
      path.join(executionRoot, 'PLD', 'scoreboard.md'),
      `# PLD Scoreboard
...
`,
    ...
    fs.mkdirSync(path.join(executionRoot, 'PLD', 'state', 'plot-mode'), {recursive: true});
    fs.writeFileSync(
      path.join(executionRoot, 'PLD', 'state', 'plot-mode', 'events.ndjson'),
      `${JSON.stringify({
        execution: 'plot-mode',
        lane: 'Lane 1',
        role: 'coordinator',
    ...
    fs.mkdirSync(path.join(cwdRoot, 'PLD', 'executions', 'plot-mode'), {recursive: true});
    fs.writeFileSync(
      path.join(cwdRoot, 'PLD', 'executions', 'plot-mode', 'lane-1.md'),
      `# Lane 1
...
`,
    ...
    const laneState = JSON.parse(
      fs.readFileSync(
        path.join(executionRoot, 'PLD', 'state', 'plot-mode', 'lane-1.json'),
        'utf8',
      ),
    );
...
test('parked replay clears stale projected fields from scoreboard projections', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-envelope-clear-'));
  const originalCwd = process.cwd();
  const originalProjectRoot = process.env.PLD_PROJECT_ROOT;
  const modulePath = path.join(originalCwd, 'plugins', 'parallel-lane-dev', 'scripts', 'pld-envelope.cjs');
...
`,
    ...
    fs.mkdirSync(path.join(root, 'PLD', 'state', 'plot-mode'), {recursive: true});
    fs.writeFileSync(
      path.join(root, 'PLD', 'state', 'plot-mode', 'events.ndjson'),
      [
        JSON.stringify({
          execution: 'plot-mode',
          lane: 'Lane 1',
    ...
    const scoreboardText = fs.readFileSync(path.join(root, 'PLD', 'scoreboard.md'), 'utf8');
...
test('review helper reduces execution state before reading projected review actions', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-review-reduce-'));
  process.env.PLD_PROJECT_ROOT = root;

  fs.mkdirSync(path.join(root, 'PLD', 'executions', 'plot-mode'), {recursive: true});
  fs.writeFileSync(
    path.join(root, 'PLD', 'executions', 'plot-mode', 'lane-3.md'),
    `# Lane 3

> Ownership family:
> \`src/render/chart.rs\`
>
> PLD worktree: \`.worktrees/lane-3-chart\`
>
> Lane-local verification:
...
`,
  ...
  const {driveReviewLoop} = freshRequire('plugins/parallel-lane-dev/scripts/pld-drive-review-loop.cjs');
...
test('commit intake helper reduces execution state before reading ready-to-commit handoff', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-intake-reduce-'));
  process.env.PLD_PROJECT_ROOT = root;

  fs.mkdirSync(path.join(root, 'PLD', 'executions', 'plot-mode'), {recursive: true});
  fs.writeFileSync(
    path.join(root, 'PLD', 'executions', 'plot-mode', 'lane-5.md'),
    `# Lane 5

> Ownership family:
> \`README.md\`
>
> PLD worktree: \`.worktrees/lane-5-docs\`
>
> Lane-local verification:
> \`npm run build\`
...
`,
  ...
  recordEnvelope(root, {
    ...
    nextExpectedPhase: 'spec-review-pending',
    'commit-title': 'docs(plot): document recovery baseline workflow',
    'commit-body': 'Align local verification and startup steps',
    timestamp: '2026-03-21T12:41:00.000Z',
  });

  const {intakeReadyToCommit} = freshRequire('plugins/parallel-lane-dev/scripts/pld-intake-ready-to-commit.cjs');
  ...
  assert.equal(result[0].proposedCommitTitle, 'docs(plot): document recovery baseline workflow');
...
test('execution insight recorder appends subagent and coordinator learnings into runtime insights journal', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-insight-'));
  process.env.PLD_PROJECT_ROOT = root;

  const {recordInsight} = freshRequire('plugins/parallel-lane-dev/scripts/pld-record-insight.cjs');
  ...
  const second = recordInsight(root, {
    ...
    detail: 'Main agent direct work can auto-commit; PLD subagents should default to READY_TO_COMMIT.',
...
test('execution insight summary helper groups actionable insights and converged schema kinds', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-insight-summary-'));
  process.env.PLD_PROJECT_ROOT = root;

  const {recordInsight} = freshRequire('plugins/parallel-lane-dev/scripts/pld-record-insight.cjs');
  const {summarizeExecutionInsights} = freshRequire('plugins/parallel-lane-dev/scripts/pld-lib.cjs');
  const {renderInsightSummary} = freshRequire('plugins/parallel-lane-dev/scripts/pld-summarize-insights.cjs');
...
test('execution insight summary separates durable global learnings from actionable lane work', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-insight-durable-'));
  process.env.PLD_PROJECT_ROOT = root;
...
test('execution insight summary keeps only the latest status for the same lane+summary key', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-insight-collapse-'));
  process.env.PLD_PROJECT_ROOT = root;
...
test('active-set replan helper updates tracked phases and lane journals atomically', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-replan-'));
  process.env.PLD_PROJECT_ROOT = root;
...
`,
  ...
  const {replanActiveSet} = freshRequire('plugins/parallel-lane-dev/scripts/pld-replan-active-set.cjs');
  ...
  const lane2State = JSON.parse(
    fs.readFileSync(path.join(root, 'PLD', 'state', 'plot-mode', 'lane-2.json'), 'utf8'),
  );
  const lane3State = JSON.parse(
    fs.readFileSync(path.join(root, 'PLD', 'state', 'plot-mode', 'lane-3.json'), 'utf8'),
  );
...
test('schedule marks clean implementing lane at same HEAD as stale-implementing', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-stale-'));
  const laneWorktree = path.join(root, '.worktrees', 'lane-1-node');
  setupTempGitRepo(laneWorktree);
  process.env.PLD_PROJECT_ROOT = root;
...
test('execution truth sync reconciles stale implementing lane back to parked state', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-sync-truth-'));
  const laneWorktree = path.join(root, '.worktrees', 'lane-1-node');
  setupTempGitRepo(laneWorktree);
  process.env.PLD_PROJECT_ROOT = root;
...
`,
  ...
  const {syncExecutionTruth} = freshRequire('plugins/parallel-lane-dev/scripts/pld-sync-execution-truth.cjs');
...
test('execution truth sync refreshes only the Current Lane Status section from lane state', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-sync-status-'));
  const laneWorktree = path.join(root, '.worktrees', 'lane-1-node');
  setupTempGitRepo(laneWorktree);
  process.env.PLD_PROJECT_ROOT = root;
  ...
  fs.mkdirSync(path.join(root, 'PLD', 'executions', 'plot-mode'), {recursive: true});
  const lanePlanPath = path.join(root, 'PLD', 'executions', 'plot-mode', 'lane-1.md');
  fs.writeFileSync(
    lanePlanPath,
    `# Lane 1 Plan - Node Contract and Handoff
...
- [x] Projected phase: implementing
- [x] Current item: stale item
- [x] Latest commit: \`old1234\`
- [x] Latest event: state-update · stale note
- [x] Next expected phase: spec-review-pending
- [x] Next refill target: stale refill
- [x] Latest note: stale note

## Refill Order

- [ ] Keep this exact refill text untouched
`,
    'utf8',
  );
...
| Execution | Lane | Ownership | Current item | Phase | Item commit | Last verification | Blocked by | Next refill target | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| plot-mode | Lane 1 | Node contract + handoff | Fresh item from lane state | parked | \`${head}\` | \`git status --short\` | none | Fresh refill target | fresh lane state note |
`,
    'utf8',
  );
  ...
  writeLaneState(root, 'plot-mode', 1, {
    ...
    lastVerification: ['git status --short'],
    blockedBy: null,
    currentItem: 'Fresh item from lane state',
    nextRefillTarget: 'Fresh refill target',
    note: 'fresh lane state note',
    correctionCount: 0,
    updatedAt: '2026-03-22T00:00:00.000Z',
  });

  const {syncExecutionTruth} = freshRequire('plugins/parallel-lane-dev/scripts/pld-sync-execution-truth.cjs');
  syncExecutionTruth(root, 'plot-mode');

  const lanePlanText = fs.readFileSync(lanePlanPath, 'utf8');
  assert.match(lanePlanText, /## Current Lane Status/);
  assert.match(lanePlanText, /- \[x\] Projected phase: parked/);
  assert.match(lanePlanText, /- \[x\] Current item: Fresh item from lane state/);
  assert.match(lanePlanText, new RegExp(`- \\[x\\] Latest commit: \\\`${head}\\\``));
  assert.match(lanePlanText, /- \[x\] Latest note: fresh lane state note/);
  assert.match(lanePlanText, /- \[ \] Keep this exact refill text untouched/);
});

test('dispatch cycle promotes the next queued work from reduced execution state', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-cycle-'));
  process.env.PLD_PROJECT_ROOT = root;
...
`,
  ...
  fs.writeFileSync(
    path.join(root, 'PLD', 'executions', 'plot-mode', 'lane-2.md'),
    `# Lane 2

> Ownership family:
> \`src/render/mod.rs\`
>
> PLD worktree: \`.worktrees/lane-2-runtime\`
>
> Lane-local verification:
> \`git status --short\`
...
`,
  ...
  const {runCycle} = freshRequire('plugins/parallel-lane-dev/scripts/pld-run-cycle.cjs');
...
test('launch helper returns assignment bundles for newly promoted lanes from reduced execution state', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-launch-'));
  process.env.PLD_PROJECT_ROOT = root;
...
`,
  ...
  const {launchActiveSet} = freshRequire('plugins/parallel-lane-dev/scripts/pld-launch-active-set.cjs');
...
test('review loop driver returns coordinator-ready bundles for review and correction phases', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-review-'));
  process.env.PLD_PROJECT_ROOT = root;
...
test('ready-to-commit intake helper returns structured coordinator commit bundle', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-intake-'));
  process.env.PLD_PROJECT_ROOT = root;
...
`,
  ...
  writeLaneState(root, 'plot-mode', 5, {
    ...
    commit: null,
    'commit-title': 'docs(plot): document recovery baseline workflow',
    'commit-body': 'Align shell/readme verification and local startup steps',
    reviewer: null,
  ...
  assert.equal(result[0].phase, 'coordinator-commit-pending');
  assert.equal(result[0].proposedCommitTitle, 'docs(plot): document recovery baseline workflow');
  assert.equal(result[0].proposedCommitBody, 'Align shell/readme verification and local startup steps');
...
test('coordinator loop combines launch, review, and commit intake into one summary', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-loop-'));
  process.env.PLD_PROJECT_ROOT = root;
...
`,
  ...
  const {runCoordinatorLoop} = freshRequire('plugins/parallel-lane-dev/scripts/pld-run-coordinator-loop.cjs');
  ...
  assert.equal(result.commitIntake[0].proposedCommitTitle, 'docs(plot): document recovery baseline workflow');
});

test('coordinator loop degrades cleanly when runtime scoreboard is malformed', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-loop-malformed-runtime-'));
  process.env.PLD_PROJECT_ROOT = root;
...
`,
  ...
  const runtimeScoreboardPath = path.join(root, 'PLD', 'state', 'scoreboard.runtime.md');
  ...
  const envelopePath = repoRoot('plugins/parallel-lane-dev/scripts/pld-envelope.cjs');
  ...
  try {
    delete require.cache[require.resolve(repoRoot('plugins/parallel-lane-dev/scripts/pld-intake-ready-to-commit.cjs'))];
    delete require.cache[require.resolve(repoRoot('plugins/parallel-lane-dev/scripts/pld-run-coordinator-loop.cjs'))];
    const {intakeReadyToCommit} = require(repoRoot('plugins/parallel-lane-dev/scripts/pld-intake-ready-to-commit.cjs'));
    const {runCoordinatorLoop} = require(repoRoot('plugins/parallel-lane-dev/scripts/pld-run-coordinator-loop.cjs'));
    ...
    assert.equal(commitIntake[0].proposedCommitTitle, 'docs(plot): document recovery baseline workflow');
...
test('coordinator loop surfaces telemetry summary and review path when present', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-loop-telemetry-'));
  process.env.PLD_PROJECT_ROOT = root;
...
`,
  ...
  const stateDir = path.join(root, 'PLD', 'state', 'plot-mode');
  fs.mkdirSync(stateDir, {recursive: true});
  fs.writeFileSync(
    path.join(stateDir, 'telemetry-summary.json'),
    `${JSON.stringify(
  ...
  assert.equal(
    packageJson.scripts['pld:telemetry:summarize'],
    'node plugins/parallel-lane-dev/scripts/pld-summarize-telemetry.cjs',
  );
  assert.equal(
    packageJson.scripts['pld:telemetry:review'],
    'node plugins/parallel-lane-dev/scripts/pld-render-telemetry-review.cjs',
  );
  assert.ok(result.telemetrySummary.minuteBuckets.length >= 1);
  assert.equal(
    result.telemetryReviewPath,
    path.join(root, 'PLD', 'state', 'plot-mode', 'telemetry-review.md'),
  );
...
test('dispatch plan helper builds a prioritized action queue from autopilot output', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-dispatch-'));
  process.env.PLD_PROJECT_ROOT = root;
...
`,
  ...
  const {buildDispatchPlan} = freshRequire('plugins/parallel-lane-dev/scripts/pld-build-dispatch-plan.cjs');
  ...
  assert.match(result.queue[0].message, /Proposed commit title: docs\(plot\): document recovery baseline workflow/);
...
test('review helper surfaces actionable execution insights alongside review actions', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-auth-pld-review-insights-'));
  process.env.PLD_PROJECT_ROOT = root;
...
`,
  ...
  const {driveReviewLoop, renderActions} = freshRequire('plugins/parallel-lane-dev/scripts/pld-drive-review-loop.cjs');
  const result = driveReviewLoop(root, 'plot-mode');
