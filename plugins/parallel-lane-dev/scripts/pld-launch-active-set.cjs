...
const {composeMessage} = require('./pld-compose-message.cjs');
const {loadLanePlan, resolveProjectRoot} = require('./pld-lib.cjs');
const {
...
function main() {
  ...
  if (!args.execution) {
    throw new Error(
      'Usage: node plugins/parallel-lane-dev/scripts/pld-launch-active-set.cjs --execution <id> [--max-active <n>] [--dry-run] [--json]',
    );
  }
