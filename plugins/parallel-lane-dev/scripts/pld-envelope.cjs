...
function eventLogPath(projectRoot, execution) {
  ...
  return path.join(resolvePldTreeDir(projectRoot), 'state', execution, 'events.ndjson');
}
...
function normalizeEnvelope(projectRoot, rawEnvelope) {
  ...
  if (!execution || !lane || !role || !eventType || !rawEnvelope.summary) {
    throw new Error(
      'execution, lane, role, eventType, and summary are required for an PLD lane handoff envelope',
    );
  }
  if (!ENVELOPE_EVENT_TYPES.includes(eventType)) {
    throw new Error(`Unknown PLD envelope eventType "${eventType}"`);
  }

  const command = rawEnvelope.command || null;
  const isCommandEvent = eventType.startsWith('command-');
  if (isCommandEvent && !command) {
    throw new Error(`command is required for PLD command telemetry eventType "${eventType}"`);
  }
...
function reduceExecution(projectRoot, execution) {
  ...
  delete require.cache[require.resolve('./pld-refresh-scoreboard.cjs')];
