...
const {computeExecutionSchedule, resolveProjectRoot} = require('./pld-lib.cjs');
const {
...
function main() {
  ...
  if (!args.execution) {
    throw new Error(
      'Usage: node plugins/parallel-lane-dev/scripts/pld-suggest-schedule.cjs --execution <id> [--max-active <n>] [--json]',
    );
  }
