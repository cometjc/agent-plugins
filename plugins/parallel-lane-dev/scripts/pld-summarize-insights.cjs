...
const {resolveProjectRoot, summarizeExecutionInsights} = require('./pld-lib.cjs');
...
function main() {
  ...
  if (!args.execution) {
    throw new Error(
      'Usage: node plugins/parallel-lane-dev/scripts/pld-summarize-insights.cjs --execution <id> [--limit <n>] [--json]',
    );
  }
