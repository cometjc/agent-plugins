...
function recordLaneState(projectRoot, args) {
  if (!args.execution || !args.lane || !args.phase) {
    throw new Error(
      'execution, lane, and phase are required to record PLD lane state',
    );
  }
  ...
  return require('./pld-lib.cjs').laneStatePath(projectRoot, args.execution, args.lane) || result.filePath;
}
...
function main() {
  ...
  if (!args.execution || !args.lane || !args.phase) {
    throw new Error(
      'Usage: node plugins/parallel-lane-dev/scripts/pld-record-lane-state.cjs --execution <id> --lane <n> --phase <phase> [--expected-next-phase <phase>] [--commit <sha>] [--commit-title <title>] [--commit-body <body>] [--reviewer <result>] [--correction-count <n>] [--verification <cmd>] [--blocked-by <reason>] [--note <text>] [--updated-at <iso>]',
    );
  }
