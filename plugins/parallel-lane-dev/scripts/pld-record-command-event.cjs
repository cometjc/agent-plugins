...
const {recordEnvelope} = require('./pld-envelope.cjs');
...
function recordCommandEvent(projectRoot, args) {
  if (!args.execution || !args.lane || !args.eventType || !args.command) {
    throw new Error(
      'execution, lane, eventType, and command are required to record PLD command telemetry',
    );
  }
  if (!COMMAND_EVENT_TYPES.includes(args.eventType)) {
    throw new Error(
      `Unknown PLD command eventType "${args.eventType}". Expected one of: ${COMMAND_EVENT_TYPES.join(', ')}`,
    );
  }
...
function main() {
  ...
  if (!args.execution || !args.lane || !args.eventType || !args.command) {
    throw new Error(
      'Usage: node plugins/parallel-lane-dev/scripts/pld-record-command-event.cjs --execution <id> --lane <n> --event-type <command-started|command-finished|command-failed|command-blocked|command-probe> --command <text> [--cwd <path>] [--status <status>] [--exit-code <n>] [--duration-ms <n>] [--block-kind <kind>] [--probe-summary <text>] [--pid <n>] [--timestamp <iso>]',
    );
  }
